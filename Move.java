...
final class Move {
     ...
     *  valid move. */
    static loa.Move mv(String s, boolean capture) {
        s = s.trim();
    ...
    static loa.Move mv(String move) {
    ...
    static loa.Move mv(Square from, Square to, boolean capture) {
    ...
    static loa.Move mv(Square from, Square to) {
    ...
    private Move(Square from, Square to, boolean capture) {
        ...
        _from = from; _to = to;
        _capture = capture;
        _captureMove = _capture ? this : new loa.Move(from, to, true);
    }
            ...
            for (int r = 0; r < BOARD_SIZE; r += 1) {
                ...
                for (int dir = 0; dir < 8; dir += 1) {
                    for (Square to = from.moveDest(dir, 1); to != null;
                        ...
                        int toi = to.index();
                        _moves[fromi][toi][0] = new loa.Move(from, to, false);
                        _moves[fromi][toi][1]
